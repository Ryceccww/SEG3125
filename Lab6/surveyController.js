...
module.exports = function(app){
    ...
    app.get('/analysis', function(req, res){
        ...
        var buy=readData("buy");
        var like = readData("like");
        var favourite = readData("favourite");
        var agree = readData("agree");
        var comments = readData("comments");
        res.render('showResults', {results: [name,buy,like,favourite,agree,comments]});
        console.log( [name,buy,like,favourite,agree,comments]);
    });
    ...
    app.post('/niceSurvey', urlencodedParser, function(req, res){
        ...
        for (var key in json){
            ...
            if ((key === "buy") && (json[key].length === 2)){
                for (var item in json[key]){
                    combineCounts(key, json[key][item]);
                }
            }
            else if ((key === "buy") && (json[key].length === 3)){
                for (var item in json[key]){
                    combineCounts(key, json[key][item]);
                }
            }
